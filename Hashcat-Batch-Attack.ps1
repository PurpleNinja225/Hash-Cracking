...
foreach ($File in $HashFile){
    &$PathToHashcatExE --attack-mode $AttackMode --opencl-device-types $DeviceType --hash-type $HashMode --workload-profile 3 --status --status-timer=60 --remove --separator : --gpu-temp-disable --outfile $OutFile --outfile-format=3 $File.FullName $PathToWordList
    }
